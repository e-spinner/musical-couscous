...
- If an availability block is `9:00 AM` to `12:00 PM`, then `blockStart` is `9:00` and `blockEnd` is `12:00`
- If a task only uses the middle two hours of that block, then its segment can have `start: 10:00` and `end: 12:00`
- If a short task only needs `45 minutes`, the segment can use just that `45-minute` slice
